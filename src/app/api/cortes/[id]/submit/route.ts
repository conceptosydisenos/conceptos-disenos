import { NextRequest, NextResponse } from "next/server"
import { db } from "@/lib/db"
import { work_cuts, audit_logs } from "@/lib/db/schema"
import { requireAuth } from "@/lib/auth"
import { eq } from "drizzle-orm"

export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const user = await requireAuth()

    const [cut] = await db
      .select({ id: work_cuts.id, status: work_cuts.status, cut_number: work_cuts.cut_number })
      .from(work_cuts)
      .where(eq(work_cuts.id, params.id))

    if (!cut) {
      return NextResponse.json({ success: false, error: "Corte no encontrado" }, { status: 404 })
    }

    if (cut.status !== "draft") {
      return NextResponse.json(
        { success: false, error: `El corte está en estado '${cut.status}' — solo se pueden enviar borradores.` },
        { status: 409 }
      )
    }

    await db
      .update(work_cuts)
      .set({ status: "submitted" })
      .where(eq(work_cuts.id, params.id))

    await db.insert(audit_logs).values({
      user_id: user.id,
      action: "update",
      entity_type: "work_cut",
      entity_id: params.id,
      old_values: { status: "draft" },
      new_values: { status: "submitted" },
    })

    return NextResponse.json({ success: true, data: { id: params.id, status: "submitted" } })
  } catch {
    return NextResponse.json({ success: false, error: "Error al enviar corte" }, { status: 500 })
  }
}
